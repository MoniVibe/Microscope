...
def write_tiff(
    ...
    meta_dict = _prepare_metadata(data, metadata, dx_um, dy_um, dz_um, wavelengths_um, na)
...
def _prepare_metadata(
    ...
    if wavelengths_um is not None:
        meta["wavelengths_um"] = (
            wavelengths_um.tolist() if hasattr(wavelengths_um, "tolist") else list(wavelengths_um)
        )
...
def read_tiff(filename: str | Path) -> tuple[np.ndarray, dict]:
...
def write_field_stack(filename: str | Path, fields: dict[str, torch.Tensor], config: dict) -> None:
    ...
    for name, field in fields.items():
        ...
        field_info.append(
            {"name": name, "shape": list(field.shape), "is_complex": field.is_complex()}
        )

...
class M3Pipeline:
    ...
    def run(self) -> dict[str, Any]:
        ...
        if solver_type == "split_step_bpm" and "grating" in self.config["propagation"]:
            ...
            if lens.get("enabled", False):
                output_field = self.solver.propagate_high_na_focus(
                    source_field, lens["focal_length"], dx
                )
            else:
                output_field = self.solver.propagate(source_field, z_distance, dx)
        else:
        ...
        metrics = calculate_m3_metrics(
            output_field, wavelength, NA, dx, reference_field=source_field, device=self.device
        )
        ...
        return {"metrics": metrics, "performance": self.perf_stats, "compliance": compliance}
    ...
    def _save_outputs(self, intensity: np.ndarray, field: torch.Tensor):
        ...
        tifffile.imwrite(tiff_path, intensity, metadata=metadata, photometric="minisblack")
...
def main():
    ...
    parser.add_argument(
        "--config", "-c", type=Path, required=True, help="Path to M3 configuration file (YAML)"
    )

    parser.add_argument(
        "--output", "--out", "-o", type=Path, default=None, dest="output", help="Output directory"
    )

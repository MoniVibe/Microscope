...
class M3Metrics:
    ...
    def calculate_all_metrics(
        self,
        field: torch.Tensor,
        reference_field: Optional[torch.Tensor] = None
    ) -> Dict[str, float]:
        """
        ...
        # Log metrics
        logger.debug(f"M3 Metrics: L2={l2_error:.4f}, Energy={energy_error:.4f}, Strehl={strehl:.4f}")
    ...
    def _calculate_strehl_ratio(self, intensity: torch.Tensor) -> float:
        ...
        # For well-corrected systems, Strehl > 0.8
    ...
    def _calculate_airy_first_zero_error(self, intensity: torch.Tensor) -> float:
        ...
        if first_zero_idx is not None and first_zero_idx < len(radii):
            ...
            # Compare with theory
            error = abs(first_zero_measured - self.airy_first_zero_theory) / self.airy_first_zero_theory
        else:
    ...
    def _find_first_minimum(self, profile: np.ndarray) -> Optional[int]:
        """
        ...
        for i in range(2, len(smoothed) - 2):
            if (smoothed[i] < smoothed[i-1] and 
                ...
                smoothed[i] < 0.5 * smoothed[0]):  # Must be significantly lower than peak
                return i

...
def fft2_with_precision(
    field: torch.Tensor, inverse: bool = False, enforce_fp32: bool = True
) -> torch.Tensor:
...
def validate_precision_invariants(
    field_in: torch.Tensor, field_out: torch.Tensor, intermediate_spectra: list[torch.Tensor] = None
) -> dict:
    ...
    """
    results = {"input_valid": True, "output_valid": True, "spectra_valid": True, "errors": []}
    ...
    results["all_valid"] = (
        results["input_valid"] and results["output_valid"] and results["spectra_valid"]
    )

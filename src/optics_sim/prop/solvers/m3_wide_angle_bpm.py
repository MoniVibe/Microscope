...
class M3WideAngleBPMSolver:
    ...
    def __init__(
        ...
        **kwargs
        ...
        logger.info(f"M3 Wide-Angle BPM solver initialized")
    ...
    def propagate(
        ...
        logger.debug(f"M3 Wide-angle: z={z_distance*1e3:.1f} mm, steps={self.n_steps}, NA={self.NA}")
        ...
        for step in range(self.n_steps):
            ...
            current_field = self._solve_implicit_step(
                current_field, numerator, denominator
            )
        ...
        if logger.isEnabledFor(logging.DEBUG):
            ...
            if abs(energy_ratio - 1.0) > 0.02:
                logger.warning(f"M3: Energy deviation {abs(1-energy_ratio)*100:.2f}% (acceptable for wide-angle)")
    ...
    def validate_m3_compliance(self) -> Dict[str, Any]:
        ...
        compliance = {
            ...
            'deterministic': torch.backends.cudnn.deterministic if torch.cuda.is_available() else True,
        ...
        compliance['m3_compliant'] = (
            compliance['fp32_enforced'] and
            compliance['mixed_fft_disabled'] and
            compliance['deterministic'] and
            compliance['matmul_precision'] == 'highest'
        )

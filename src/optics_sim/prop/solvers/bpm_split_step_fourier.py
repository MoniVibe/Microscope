...
def run(
    ...
    pml_thickness = (
        ...
        else plan.get("pml_thickness_px", 16)
    )
    ...
    band_mask = _create_band_limit_mask(
        ny, nx, dx, dy, na_max, wavelengths[0], n.mean().item(), device
    )
...
def _create_band_limit_mask(
    ...
    mask = torch.where(
        ...
        torch.where(
            ...
            * (
                1
                + torch.cos(np.pi * (f_radial - f_max + transition_width) / (2 * transition_width))
            ),
        ),
    )
...
def _split_step_fourier(
    ...
    if na_max > 0.8:  # Only for high-NA
        decay = torch.where(~is_propagating, torch.exp(-kz * dz), torch.ones_like(kz))
    else:
...
def adaptive_propagate(
    field: torch.Tensor, plan: dict, error_threshold: float = 0.01
) -> torch.Tensor:

...
def make_plan(cfg: Dict) -> Plan:
    ...
    if plan.memory_estimate_gb > vram_gb:
        print(f"Warning: Estimated memory {plan.memory_estimate_gb:.2f} GB exceeds budget {vram_gb:.1f} GB")
...
def _compute_z_steps(cfg: Dict, dx: float, lambda_min: float, 
                    na_max: float, preset: str) -> List[float]:

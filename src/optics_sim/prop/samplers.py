...
def _resample_fourier(
    field: torch.Tensor, ny_new: int, nx_new: int, anti_alias: bool = True
) -> torch.Tensor:
    ...
    if anti_alias and (ny_new < ny_old or nx_new < nx_old):
        ...
        filter_2d = 1.0 / (
            1.0 + (torch.abs(fy_grid / fy_cut) ** 6 + torch.abs(fx_grid / fx_cut) ** 6)
        )
    ...
    if ny_new != ny_old or nx_new != nx_old:
        ...
        if ny_new > ny_old or nx_new > nx_old:
            ...
            padding = [
                max(0, pad_x),
                max(0, nx_new - nx_old - pad_x),
                max(0, pad_y),
                max(0, ny_new - ny_old - pad_y),
            ]
...
def _resample_spatial(
    field: torch.Tensor, ny_new: int, nx_new: int, mode: str = "bilinear", anti_alias: bool = True
) -> torch.Tensor:
    ...
    resampled_real = F.interpolate(
        field_real, size=(ny_new, nx_new), mode=mode, align_corners=False
    )
    resampled_imag = F.interpolate(
        field_imag, size=(ny_new, nx_new), mode=mode, align_corners=False
    )
...
def check_nyquist(field: torch.Tensor, pitch_um: float, wavelength_um: float, na: float) -> bool:
...
def compute_bandwidth(field: torch.Tensor, pitch_um: float) -> tuple[float, float]:
...
def adaptive_resample(
    field: torch.Tensor, from_pitch_um: float, to_pitch_um: float, wavelength_um: float, na: float
) -> torch.Tensor:

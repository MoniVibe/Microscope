...
def l2_field_error(
    field_computed: torch.Tensor, field_reference: torch.Tensor, normalize: bool = True
) -> float:
...
def energy_conservation(
    field_in: torch.Tensor, field_out: torch.Tensor, dx: float = 1.0, dy: float = 1.0
) -> float:
...
def strehl_ratio(
    psf: torch.Tensor, psf_ideal: torch.Tensor | None = None, normalize: bool = True
) -> float:
...
def mtf_cutoff(
    field: torch.Tensor, dx: float, dy: float, threshold: float = 0.1
) -> tuple[float, float]:
...
def compute_fwhm(psf: torch.Tensor, dx: float, dy: float) -> tuple[float, float]:
...
def phase_rmse(
    phase_computed: torch.Tensor, phase_reference: torch.Tensor, unwrap: bool = False
) -> float:
...
def beam_quality_m2(
    field: torch.Tensor, z_positions: torch.Tensor, dx: float, dy: float, wavelength: float
) -> tuple[float, float]:

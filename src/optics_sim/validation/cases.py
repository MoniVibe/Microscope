...
def high_na_reference(
    wavelength_um: float, na: float, z_um: float, nx: int = 256, ny: int = 256
) -> torch.Tensor:
    ...
    if na > 0.5:
        obliquity = torch.where(is_propagating, torch.sqrt(torch.abs(kz / k)), torch.zeros_like(kz))
...
def validate_case(
    ...
    # Check pass/fail
    passed = (
        l2_err <= thresholds["l2_error"]
        and energy_err <= thresholds["energy_conservation"]
        and phase_err <= thresholds["phase_rmse"]
    )

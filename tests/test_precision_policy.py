...
# Named tolerances for PLR2004 in precision tests
L2_TOL = 0.03
PARITY_TOL = 0.01
...
def test_as_multi_slice_cpu_fp64_intermediates():
    ...
    E0, Ez_analytical = gaussian_free_space(
        wavelength_um, waist_um, z_um, nx, ny, plan["dx_um"], plan["dy_um"]
    )
    ...
    assert l2_err < L2_TOL, f"L2 error {l2_err:.3%} exceeds 3% (CPU FP64 path)"
...
def test_cpu_gpu_parity():
    ...
    E0_cpu, Ez_analytical = gaussian_free_space(wavelength_um, waist_um, z_um, nx, ny, dx, dy)
    ...
    for solver, name in solvers:
        ...
        assert l2_cpu <= L2_TOL, f"{name} CPU L2 error {l2_cpu:.3%} exceeds 3%"
        assert l2_gpu <= L2_TOL, f"{name} GPU L2 error {l2_gpu:.3%} exceeds 3%"

        # CPU and GPU should be close to each other
        cpu_gpu_diff = l2_field_error(Ez_gpu.cpu(), Ez_cpu)
        assert (
            cpu_gpu_diff <= PARITY_TOL
        ), f"{name} CPU-GPU difference {cpu_gpu_diff:.3%} exceeds 1%"
